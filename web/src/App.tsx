...
function InteractionCard({ interaction, waiting, onRespond }: { interaction: InteractionRequest; waiting: number; onRespond: (interaction: InteractionRequest, value: string) => Promise<void> }) {
  ...
  return <div className={`interaction-card${choiceClass}`}>
    <header><strong>{interaction.kind === 'approval' ? `Allow ${interaction.toolName}?` : interaction.question}</strong>{waiting > 0 && <span>{waiting} waiting</span>}</header>
    {interaction.kind === 'approval' && <pre>{JSON.stringify(interaction.arguments, null, 2)}</pre>}
    {answerInput && <input value={answer} onChange={event => setAnswer(event.target.value)} placeholder="Type another answer" autoFocus={customAnswer}/>}
    <div>{interaction.kind === 'approval'
      ...
      : interaction.choices === undefined || customAnswer
        ? <><button className="primary" disabled={answer.trim().length === 0} onClick={() => void onRespond(interaction, answer.trim())}>Answer</button></>
        : <>{interaction.choices.map(choice => <button key={choice} onClick={() => void onRespond(interaction, choice)}>{choice}</button>)}<button className="other-choice" onClick={() => setCustomAnswer(true)}>Other…</button></>}
    </div>
  </div>
}
...
function RunView({ snapshot, workspace, live, liveTools, interactions, error, onSend, onPause, onResume, onRespond }: {
  ...
  return <main className="run-view">
    ...
    <div className="composer-wrap"><div className="composer"><textarea value={draft} disabled={session?.writable !== true} onChange={event => setDraft(event.target.value)} onKeyDown={event => { if (event.key !== 'Enter' || event.shiftKey || event.nativeEvent.isComposing) return; event.preventDefault(); void send() }} placeholder={session?.writable ? 'Ask Knot to inspect or change the workspace…' : 'This session is read only'} aria-label="Message. Press Enter to send or Shift+Enter for a new line." rows={3}/><div className="composer-actions"><div>{session?.runState === 'running' ? <button className="small-action" onClick={() => setDelivery(value => value === 'steer' ? 'follow_up' : 'steer')}>{delivery === 'steer' ? 'Steer now' : 'Follow up'}</button> : <button className="small-action" disabled>New turn</button>}<button className="small-action" onClick={() => setDraft(value => `${value}@`)}>@ Files</button></div><div>{session?.runState === 'paused' ? <button className="pause-button" onClick={() => void onResume()}><Icon name="play" size={14}/>Resume</button> : <button className="pause-button" disabled={session?.runState !== 'running'} onClick={() => void onPause()}><Icon name="pause" size={14}/>Pause</button>}<button className="send-button" disabled={draft.trim().length === 0 || session?.writable !== true} onClick={() => void send()}><Icon name="send" size={15}/></button></div></div></div><div className="fixture-note">{queued === undefined ? session?.writable ? 'Enter to send · Shift+Enter for a new line · completed facts are committed to JSONL' : 'Read-only persisted Journal' : `Queued follow-up: ${queued}`}</div></div>
  </main>
}
