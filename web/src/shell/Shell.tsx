...
import type { ApprovalMode, ProviderProfileDraft, ProviderProfileSummary, ReasoningEffort, SessionSummary, StudioAssembly, StudioCase } from '../api/workbench-api'
import type { DialogKind, Mode, ProjectView } from '../app/types'
...
export function WorkbenchHeader({ mode, project, session, currentCase, workspace, model, inspectorOpen, onModel, onSettings, onInspector }: {
  readonly mode: Mode; readonly project: ProjectView; readonly session?: SessionSummary; readonly currentCase: StudioCase; readonly workspace: string; readonly model: string; readonly inspectorOpen: boolean; readonly onModel: () => void; readonly onSettings: () => void; readonly onInspector: () => void
}) {
  const { t } = useI18n()
...
export function Dialog({ kind, project, projects, currentCase, assemblies, providerProfiles, providerProfileId, onClose, onCreateSession, onCreateProvider, onCreateCase, onCreateProject, onSelectProject, onProviderProfile }: { readonly kind: DialogKind; readonly project: ProjectView; readonly projects: readonly ProjectView[]; readonly currentCase: StudioCase; readonly assemblies: readonly StudioAssembly[]; readonly providerProfiles: readonly ProviderProfileSummary[]; readonly providerProfileId: string; readonly onClose: () => void; readonly onCreateSession: (title: string, cwd: string, providerProfileId: string, reasoningEffort: ReasoningEffort | undefined, approvalMode: ApprovalMode, projectId: string) => void; readonly onCreateProvider: (input: ProviderProfileDraft) => void; readonly onCreateCase: (title: string, projectId: string) => void; readonly onCreateProject: (name: string, root: string, assemblyId: string) => void; readonly onSelectProject: (id: string) => void; readonly onProviderProfile: (providerProfileId: string) => void }) {
  const { t } = useI18n()
  ...
  const [cwd, setCwd] = useState(kind === 'projects' ? '.' : currentCase.workspace)
  ...
  const [assemblyId, setAssemblyId] = useState(project.assembly.id)
  const [adapter, setAdapter] = useState<ProviderProfileDraft['adapter']>('openai-compatible')
  const [newProviderLabel, setNewProviderLabel] = useState('')
  const [newProviderModel, setNewProviderModel] = useState('')
  const [newProviderUrl, setNewProviderUrl] = useState('')
  const [newProviderKey, setNewProviderKey] = useState('')
  const [newProviderWindow, setNewProviderWindow] = useState('')
  ...
  const selectedAssembly = assemblies.find(item => item.id === assemblyId)
  const assemblySelect = <label>{t('dialog.assembly')}<select value={assemblyId} onChange={event => setAssemblyId(event.target.value)}>{assemblies.map(assembly => <option key={assembly.id} value={assembly.id}>{assembly.title}</option>)}</select></label>
  ...
  const providerDraftValid = newProviderLabel.trim() !== '' && newProviderModel.trim() !== ''
    && (adapter === 'deepseek' ? newProviderKey.trim() !== '' : newProviderUrl.trim() !== '')
    && (newProviderWindow.trim() === '' || Number.isInteger(Number(newProviderWindow)) && Number(newProviderWindow) > 0)
  const addProvider = () => onCreateProvider({
    label: newProviderLabel.trim(), adapter, model: newProviderModel.trim(),
    ...(newProviderUrl.trim() === '' ? {} : { baseUrl: newProviderUrl.trim() }),
    ...(newProviderKey.trim() === '' ? {} : { apiKey: newProviderKey.trim() }),
    ...(newProviderWindow.trim() === '' ? {} : { contextWindow: Number(newProviderWindow) }),
  })
  return <div className="dialog-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) onClose() }}><section className="dialog"><header><div><span className="eyebrow">{kind === 'settings' || kind === 'projects' ? t('dialog.project') : kind === 'plugin-library' ? t('dialog.studio') : t('dialog.create')}</span><h2>{heading}</h2></div><button className="icon-button" onClick={onClose}><Icon name="close" size={16}/></button></header>{kind === 'new-session' && <><label>{t('dialog.sessionTitle')}<input value={title} onChange={event => setTitle(event.target.value)}/></label><label>{t('dialog.workingDirectory')}<input value={cwd} onChange={event => setCwd(event.target.value)}/></label>{providerSelect}{reasoningSelect}{approvalSelect}<div className="dialog-summary"><span>{t('dialog.assembly')}</span><strong>{project.assembly.title}</strong><span>{t('dialog.model')}</span><strong>{selectedProvider?.model ?? t('dialog.noProvider')}</strong><span>{t('dialog.reasoning')}</span><strong>{reasoningEffort ?? t('dialog.providerDefault')}</strong><span>{t('dialog.approval')}</span><strong>{approvalMode}</strong></div><footer><button onClick={onClose}>{t('dialog.cancel')}</button><button className="primary" disabled={title.trim() === '' || cwd.trim() === '' || selectedProvider?.configured !== true} onClick={() => onCreateSession(title.trim(), cwd.trim(), providerProfileId, reasoningEffort, approvalMode, project.id)}>{t('dialog.createSession')}</button></footer></>}{kind === 'new-case' && <><label>{t('dialog.caseName')}<input value={title} onChange={event => setTitle(event.target.value)}/></label><p className="dialog-copy">{t('dialog.caseDescription')}</p><footer><button onClick={onClose}>{t('dialog.cancel')}</button><button className="primary" disabled={title.trim() === ''} onClick={() => onCreateCase(title.trim(), project.id)}>{t('dialog.createCase')}</button></footer></>}{kind === 'projects' && <><div className="project-list">{projects.map(item => <button key={item.id} className={item.id === project.id ? 'active' : ''} onClick={() => onSelectProject(item.id)}><span className="project-avatar">{item.name.slice(0, 1).toUpperCase()}</span><span><strong>{item.name}</strong><small>{item.root}</small></span>{item.id === project.id && <Icon name="check" size={14}/>}</button>)}</div><div className="dialog-divider"><span>{t('dialog.newFixtureProject')}</span></div><label>{t('dialog.projectName')}<input value={title} onChange={event => setTitle(event.target.value)}/></label><label>{t('dialog.workspaceRoot')}<input value={cwd} onChange={event => setCwd(event.target.value)}/></label>{assemblySelect}<footer><button onClick={onClose}>{t('dialog.cancel')}</button><button className="primary" disabled={title.trim() === '' || cwd.trim() === '' || selectedAssembly === undefined} onClick={() => onCreateProject(title.trim(), cwd.trim(), assemblyId)}>{t('dialog.createProject')}</button></footer></>}{kind === 'settings' && <><div className="settings-list"><div><span>{t('dialog.projectRoot')}</span><code>{project.root}</code></div><div><span>{t('dialog.runtime')}</span><strong>{t('dialog.runtimeValue')}</strong></div><div><span>{t('dialog.storage')}</span><strong>{t('dialog.storageValue')}</strong></div><div><span>{t('dialog.defaultCase')}</span><strong>{currentCase.title}</strong></div></div>{providerSelect}<details className="provider-editor"><summary>{t('dialog.addProvider')}</summary><label>{t('dialog.providerAdapter')}<select value={adapter} onChange={event => setAdapter(event.target.value as ProviderProfileDraft['adapter'])}><option value="openai-compatible">OpenAI-compatible</option><option value="deepseek">DeepSeek</option></select></label><label>{t('dialog.providerLabel')}<input value={newProviderLabel} onChange={event => setNewProviderLabel(event.target.value)}/></label><label>{t('dialog.model')}<input value={newProviderModel} onChange={event => setNewProviderModel(event.target.value)}/></label><label>{t('dialog.baseUrl')}<input value={newProviderUrl} placeholder={adapter === 'deepseek' ? 'https://api.deepseek.com' : 'https://example.com/v1'} onChange={event => setNewProviderUrl(event.target.value)}/></label><label>{t('dialog.apiKey')}<input type="password" autoComplete="off" value={newProviderKey} onChange={event => setNewProviderKey(event.target.value)}/></label><label>{t('dialog.contextWindow')}<input inputMode="numeric" value={newProviderWindow} onChange={event => setNewProviderWindow(event.target.value)}/></label><button disabled={!providerDraftValid} onClick={addProvider}>{t('dialog.saveProvider')}</button><small>{t('dialog.providerSecretHint')}</small></details><footer><button className="primary" onClick={onClose}>{t('dialog.done')}</button></footer></>}{kind === 'plugin-library' && <><div className="library-grid">{plugins.slice(2, 8).map(plugin => <button key={plugin.id} onClick={onClose}><span className={`category ${plugin.category}`}>{plugin.category}</span><strong>{plugin.name}</strong><small>{plugin.responsibility}</small><i>{t('dialog.alreadyAssembled')}</i></button>)}</div><p className="dialog-copy">{t('dialog.libraryDescription')}</p></>}</section></div>
}
