...
const business: readonly Definition[] = [
  { metadata: { id: 'system-prompt', name: 'CodingSystemPrompt', category: 'context', responsibility: 'Install the stable coding instruction once.', listens: ['session.start'], emits: ['system.prompt'], source: 'src/cases/case2/system-prompt.ts' }, create: () => codingSystemPromptPlugin() },
  ...
  { metadata: { id: 'tools', name: 'Tools', category: 'effect', responsibility: 'Execute one tool-call batch and return every outcome.', listens: ['tool.call'], emits: ['tool.result'], source: 'src/cases/case1/tools.ts' }, create: context => toolsPlugin(context.tools) },
