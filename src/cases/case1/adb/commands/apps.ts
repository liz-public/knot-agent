...
export function appHandlers(executor: AdbExecutor, options: AppHandlerOptions = {}): Readonly<Record<string, ToolHandler>> {
  return {
    ...
    async set_alarm_clock(arguments_) { const hour = Number(arguments_['hour']); const minute = arguments_['minute'] === undefined ? 0 : Number(arguments_['minute']); if (!Number.isInteger(hour) || hour < 0 || hour > 23 || !Number.isInteger(minute) || minute < 0 || minute > 59) return err('invalid_time'); await executor.shell(`am start -a android.intent.action.SET_ALARM --ei android.intent.extra.alarm.HOUR ${hour} --ei android.intent.extra.alarm.MINUTES ${minute}`); return ok({ hour, minute }, '已向系统发起设置闹钟请求。') },
