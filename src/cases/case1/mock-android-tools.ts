...
export interface MockAndroidCliOptions {
...
export function createMockAndroidDispatcher(
  session: AndroidDeviceSession,
  options: MockAndroidCliOptions = {},
): ToolDispatcher {
  ...
  return createToolDispatcher({
    ...
    contact(arguments_) {
      ...
      if ((sub !== 'lookup' && sub !== 'call') || typeof query !== 'string' || query.length === 0) {
        return err('invalid_contact_request')
      }
      session.pendingContact = query
      const value = { source: 'contact', candidates: [{ ordinal_1based: 1, display_name: query }] }
      return ok(
        { action: 'candidates', intent: sub, candidates: value.candidates },
