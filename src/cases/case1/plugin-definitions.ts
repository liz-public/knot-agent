import type { PluginMetadata, PluginNode } from '../../assembly-definition.js'
...
interface Definition {
  ...
  readonly create: (context: BuildContext) => Plugin
}
...
const business: readonly Definition[] = [
  { metadata: { id: 'system-prompt', name: 'SystemPrompt', category: 'context', responsibility: 'Install the stable mobile-assistant instruction once.', listens: ['session.start'], emits: ['system.prompt'], source: 'src/cases/case1/system-prompt.ts' }, create: () => systemPromptPlugin() },
  ...
  { metadata: { id: 'tools', name: 'Tools', category: 'effect', responsibility: 'Execute one tool-call batch and return every outcome.', listens: ['tool.call'], emits: ['tool.registry', 'tool.result'], source: 'src/cases/case1/tools.ts' }, create: context => toolsPlugin(context.tools) },
